...
namespace ticketing_api.Services
{
    public class OrderService
    {
        ...
        public async Task<PagingResults<OrderView>> GetOrderViewAsync(AppUser appUser, SieveModel sieveModel, bool tickets = false)
        {
            var query = OrderViewQuery;

            query = tickets ? query.Where(o => o.OrderStatusId.Id != AppConstants.OrderStatuses.Preticket).AsQueryable() 
                : query.Where(o => o.OrderStatusId.Id == AppConstants.OrderStatuses.Preticket).AsQueryable();
            ...
            switch (role)
            {
                ...
                case "SALES":
                    ...
                    query = query.Where(x => x.SalesRepId.Id == sales.Id && x.OrderStatusId.Id == AppConstants.OrderStatuses.Assigned).AsQueryable();
                    break;
            }
