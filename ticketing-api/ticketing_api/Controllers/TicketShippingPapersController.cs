...
namespace ticketing_api.Controllers
{
    public partial class TicketsController : BaseController.BaseController
    {
        ...
        public async Task<IActionResult> GetShippingPaper([FromRoute] int id, [FromQuery]decimal startingGallons, [FromQuery]string paperSize)
        {
            var isPermisssion = await CheckPermissions("TICKETS", "Update");

            if (!isPermisssion)
            {
                return BadRequest("Generate shippingpaper pdf permission not allowed");
            }
        ...
        public async Task<IActionResult> GetShippingPaperUrl([FromRoute] int id, [FromQuery]decimal startingGallons, [FromQuery]string paperSize)
        {
            var isPermisssion = await CheckPermissions("TICKETS", "Update");

            if (!isPermisssion)
            {
                return BadRequest("Generate shippingpaper pdf and return path permission not allowed");
            }
        ...
        public async Task<IActionResult> GetShippingPaperImage([FromRoute] int id)
        {
            var isPermisssion = await CheckPermissions("TICKETS", "Update");

            if (!isPermisssion)
            {
                return BadRequest("Read shippingpaperimage permission not allowed");
            }
        ...
        public async Task<IActionResult> PostShippingPaperImageAsync([FromRoute] int id, IFormFile file)
        {
            ...
            if (!isPermisssion)
            {
                return BadRequest("Upload shippingpaperimage permission not allowed");
            }
